...
public class GripController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            if (ConnectedObject.GetComponent<MyInteractable>().touchCount == 0)//if our object isn't touching anything
            {
            ...
            else if (ConnectedObject.GetComponent<MyInteractable>().touchCount > 0)//if it is touching something 
            {
        ...
        {
            if (ToggleGripButton.GetStateDown(Hand))//check if we want to pick somthing up
            {
                Grip();
            }
        }
    ...
    private void Grip()
    {
        ...
        if (NewObject != null)
            ConnectedObject = ClosestGrabbable();//find the Closest Grabbable and set it to the connected object if it isn't null
    }
